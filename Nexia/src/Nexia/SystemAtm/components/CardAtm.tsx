/* Hooks */
import { useEffect, useState } from "react"

/* Components */
import { LoadingIcon } from "../../components/loading.ldrs"

/* Services */
import { getUserById } from "../../services/external/user/user.service"
import { getBank } from "../../services/external/bank/bank.service"



/* Render card */
type CardRenderProps = {
  enterCard: boolean
  setEnterCard: React.Dispatch<React.SetStateAction<boolean>>
  setCardNumber: (n: string) => void
}
export const CardRender = ({
  enterCard,
  setEnterCard,
  setCardNumber
}: CardRenderProps) => {

  const [isAnimating, setIsAnimating] = useState(false)
  
  // Toggle access card
  const toggleKeycard = () => {
    // Wait animation
    if (isAnimating) return

    setIsAnimating(true)
    setEnterCard(prev => !prev)

    setTimeout(() => {
      setIsAnimating(false)
    }, 1500)
  }


  return (
    <button className={`card-position-atm ${enterCard ? "in" : "out"}`} onClick={toggleKeycard}>
          
      <CardAtm setCardNumber={setCardNumber} />
      
    </button>
  )
}



/* Key card */
type data = {
  name: string,
  lastname: string
}
type dataBank = {
  card: string
}
type CardAtmProps = {
  setCardNumber: (n: string) => void
}
export const CardAtm = ({ setCardNumber }: CardAtmProps) => {
  const [data, setData] = useState<data | null>(null)
  const [dataBank, setDataBank] = useState<dataBank | null>(null)

  const date = new Date()


  // Get user and bank
  const getData = async () => {
    try {

      const res = await getUserById()

      const bank = await getBank()

      setData(res.data)

      setDataBank(bank.data.bank)

      setCardNumber(bank.data.bank?.card || "")

    } catch (error: any) {

      throw new Error(error)
      
    }
  }

  useEffect(() => {
    getData()
  }, [])



  return (
    <>
      <div className="flip-card">
        <div className="flip-card-inner">

          <div className="flip-card-front">

            {data && dataBank ? (
              <>
                <p className="heading_8264">MASTERCARD</p>
    
                <svg className="logo" x="0px" y="0px" width="36" height="36" viewBox="0 0 48 48">
    
                <path fill="#ff9800" d="M32 10A14 14 0 1 0 32 38A14 14 0 1 0 32 10Z"></path><path fill="#d50000" d="M16 10A14 14 0 1 0 16 38A14 14 0 1 0 16 10Z"></path><path fill="#ff3d00" d="M18,24c0,4.755,2.376,8.95,6,11.48c3.624-2.53,6-6.725,6-11.48s-2.376-8.95-6-11.48 C20.376,15.05,18,19.245,18,24z"></path>
                </svg>
    
                <svg version="1.1" className="chip" id="Layer_1"x="0px" y="0px" width="30px" height="30px" viewBox="0 0 50 50">
                  <image 
                    id="image0" 
                    width="50" 
                    height="50" 
                    x="0" 
                    y="0" 
                    href="data:image/png;base64," 
                    />
                </svg>
    
                <svg version="1.1" className="contactless" id="Layer_1" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 50 50">
                <image 
                  id="image0" 
                  width="50" 
                  height="50" 
                  x="0" 
                  y="0" 
                  href="data:image/png;base64," 
                  />
                </svg>
    
                <p className="number">
                  {dataBank?.card.match(/.{1,4}/g)?.join(" ")}
                </p>
    
                <p className="valid_thru">VALID THRU</p>
    
                <p className="date_8264">{`${String(7).padStart(2, "0")}/${date.getFullYear() + 3}`}</p>
    
                <p className="name">{data?.name} {data?.lastname}</p>
              </>
            ) : (
              <div>
                <LoadingIcon color="white" />
              </div>
            )}

          </div>
        
        </div>
      </div>
    </>
  )
}